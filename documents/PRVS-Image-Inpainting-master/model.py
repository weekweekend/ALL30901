...
from modules.Losses import AdversarialLoss, Hog_descriptor
...
class PRVSModel():
    ...
    def train(self, train_loader, save_path, finetune = False, iters=450000):
        ...
        while self.iter<iters:
            for items in train_loader:
                gt_images, grey_image, mask_edge, gt_edges, masks = self.__cuda__(*items)
                masked_images = gt_images * masks[:,0:1,:,:]
                masked_edges = gt_edges * masks[:,0:1,:,:]
                masks = torch.cat([masks]*3, dim = 1)
                masks = masks[:,:3,:,:]
                self.grey = grey_image
                self.forward(masked_images, masks, masked_edges,mask_edge, gt_images, gt_edges)
                ...
                if self.iter % 100 == 0:
                    ...
                    f.write("Iteration:%d, l1_loss:%.4f, time_taken:%.2f\n" %(self.iter, self.l1_loss_val/50, int_time))
                    f.close()
    ...
    def test(self, test_loader, result_save_path):
        ...
        for items in test_loader:
            ...
            fake_B, _, _, _ = self.G(masked_images, masks, mask_edge, masked_edges)
            ...
            for k in range(comp_B.size(0)):
                ...
                # file_path = '{:s}/org_img_{:d}.png'.format(result_save_path, count)
                # save_image(gt_images[k:k+1], file_path)
                file_path = '{:s}/mask_{:d}.png'.format(result_save_path, count)
                save_image(1-masks[k:k+1], file_path)
    
    def forward(self, masked_image, mask, masked_edge, mask_edge, gt_image, gt_edge):
        self.real_A = masked_image
        self.real_B = gt_image
        self.mask = mask
        self.mask_edge = mask_edge # 加的边界
        self.edge_gt = gt_edge
        fake_B, _, edge_small, edge_big = self.G(masked_image, mask, masked_edge, mask_edge)
        self.fake_B = fake_B
        self.edge_fake = [edge_small, edge_big]
        self.comp_B = self.fake_B * (1 - mask) + self.real_B * mask
        if self.iter % 1000 == 0:
            file_test_path = 'testim/test_{:d}.png'.format(self.iter)
            save_image(self.comp_B,file_test_path)
    ...
    def get_g_loss(self):
        ...
        gradient_loss = self.gradient_loss(real_B, fake_B)
        # valid_gradient_loss = self.gradient_loss(real_B, fake_B, self.mask)
        # hole_gradient_loss = self.gradient_loss(real_B, fake_B, (1 - self.mask))
        ...
        loss_G = (  tv_loss * 0.1
                  ...
                  + valid_loss * 50
                  + gradient_loss * 5
                #   + valid_gradient_loss * 5
                #   + hole_gradient_loss * 5
                  + hole_loss * 50
                  ) + 0.1 * adv_loss
    ...
    def gradient_loss(self, f1, f2, mask = 1):
        # save_image(f1[1,0,:,:],"src/img10.png")
        # save_image(f2[1,0,:,:],"src/img20.png")
        # save_image(f1[1,1,:,:],"src/img11.png")
        # save_image(f2[1,1,:,:],"src/img21.png")
        # save_image(f1[1,2,:,:],"src/img12.png")
        # save_image(f2[1,2,:,:],"src/img22.png")
        f11 = (f1[:,0,:,:]*0.299 + f1[:,1,:,:]*0.587 + f1[:,2,:,:]*0.144)
        f22 = (f2[:,0,:,:]*0.299 + f2[:,1,:,:]*0.587 + f2[:,2,:,:]*0.144)
        f1a = f11.data.cpu().numpy()
        f2b = f22.data.cpu().numpy()
        sum = 0
        for i in range(f1.shape[0]):
            fa = f1a[i,:,:]
            fb = f2b[i,:,:]
            fb = (fb-fb.min())/(fb.max()-fb.min())
            hog1 = Hog_descriptor(fa, cell_size=16, bin_size=8)
            hog2 = Hog_descriptor(fb, cell_size=16, bin_size=8)
            _, image1 = hog1.extract()
            _, image2 = hog2.extract()
            # io.imsave("src/img1.png", image1)
            # io.imsave("src/img2.png", abs(image1-image2))
            sum += abs((image1-image2)).mean()
        # print(sum / f1.shape[0] ) 
        return sum / f1.shape[0]
