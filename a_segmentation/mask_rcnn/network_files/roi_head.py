...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    # 计算预测mask与真实gt_mask之间的 BCELoss
    mask_loss = F.binary_cross_entropy_with_logits(
        mask_logits[torch.arange(labels.shape[0], device=labels.device), labels], mask_targets
    )

    # aaa = mask_targets[0].cpu().reshape((28,28))
    # bbb = mask_logits[torch.arange(labels.shape[0], device=labels.device), labels][0].cpu().detach().numpy().reshape((28,28))
    # beforea=aaa
    # beforeb=bbb
    # aaa[aaa>=0.9] = 1
    # aaa[aaa<0.9]=0
    ...
    # loss_block = ccc.sum()/(aaa + bbb).sum()

    # aaa = np.array(aaa).astype( np.uint8 )*255
    # bbb = bbb.astype( np.uint8 )*255
    # cannya = canny(np.array(aaa),sigma=1)*1.0
    # cannyb = canny(bbb,sigma=1)*1.0
    # ccc = cannya-cannyb
    # ccc[ccc<0] = 1;
    # loss_edge = ccc.sum()/(cannya+cannyb).sum()

    # tm = int(time.time())
    
    # pth1 = 'rest/block/aaa{:d}.jpg'.format(tm)
    # pth2 = 'rest/block/bbb{:d}.jpg'.format(tm)
    # pth3 = 'rest2/predict/aaa{:d}.jpg'.format(tm)
    # pth4 = 'rest2/predict/bbb{:d}.jpg'.format(tm)
