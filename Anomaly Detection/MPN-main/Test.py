...
def dismap(x,y,landa, name='dismap'):
    ...
    x = x.mean(1)
    y = y.mean(1)
    # y = (x-x.min())/(x.max()-x.min())*255
    # print(x.min(),x.max(),y.min(),y.max())
    # file_path2 = 'results/testa.jpg'
    # io.imsave(file_path2, y.reshape(256,256))
    # aaa
    res = x[0] + y[0]*landa
    res=res*100
    ...
    print(res.shape)
    # import pdb;pdb.set_trace()
    file_path2 = 'results/score/{:0>3}.jpg'.format(name)
    io.imsave(file_path2, res)
    df = pd.DataFrame(res)
    sns.heatmap(df)
    plt.savefig('results/dismap/{:0>3}.png'.format(name))
    # aaa
    plt.close()
    return True
...
parser.add_argument('--model_dir', type=str,  default='pths/log_3_bs/model_500.pth', help='directory of model')
...
test_folder = args.dataset_path+args.dataset_type+"/mine/test"
...
for video in sorted(videos_list):
    video_name = video.split('/')[-1]
    videos[video_name]['labels'] = labels[4+label_length:videos[video_name]['length']+label_length]
    labels_list = np.append(labels_list, labels[args.t_length+args.K_hots-1+label_length:videos[video_name]['length']+label_length])
...
with torch.no_grad():
    for k,(imgs) in enumerate(test_batch):
        ...
        start_t = time.time()
        # aaaa=model.forward(imgs[:,:3*4], update_weights, False)
        # print("aaaa",len(aaaa))

        outputs, fea_loss = model.forward(imgs[:,:3*4], update_weights, False)
        # 训练模型中的输出
        # outputs, _, _, _, fea_loss, _, dis_loss  = model.forward(imgs[:,:3*4], update_weights, False)
        
        # file_path1 = 'results/testa-{:d}.jpg'.format(k)
        # save_image(imgs[:,12:15].reshape((3,256,256)), file_path1)
        # print(imgs[:,12:15].shape)
        # a
        ...
        # file_path2 = 'results/testb-{:d}.jpg'.format(k)
        # save_image(mse_imgs.reshape(3,256,256)*100, file_path2)----------------------------------------------
  
        mse_feas = fea_loss.mean(-1)
        mse_feas = mse_feas.reshape((-1,1,576,768))
        # file_path2 = 'results/testb-{:d}.jpg'.format(k)
        # save_image((outputs-imgs[:,-3:]).reshape((3,256,256))*50, file_path2)
        
        ## 只保留特征图中最亮的部分
        # mseFeas =  mse_feas.reshape(1,256,256)
        # maxx = mseFeas.max()*0.70

        # mseFeas[mseFeas<maxx]=0
        # mseFeas[mseFeas >= maxx]=255

        # file_path2 = 'results/testb-{:d}.jpg'.format(k)
        # save_image(mseFeas, file_path2)

        # shape在view前后 ([1, 3, 576, 768])，([1, 1327104]), ([1])
        aa = mse_imgs
        mse_imgs = mse_imgs.view((mse_imgs.shape[0],-1))
        mse_imgs = mse_imgs.mean(-1)
        
        # shape在view前后 ([1, 1, 576, 768])，([1, 442368]), ([1])
        bb = mse_feas
        mse_feas = mse_feas.view((mse_feas.shape[0],-1))
        mse_feas = mse_feas.mean(-1)

        # 异常分布可视化
        dismap(aa,bb,0, str(k))
...
# accuracy_total = 100*AUC(anomaly_score_total, np.expand_dims(1-np.array(labels_list), 0))

accuracy_total = 888
print('The result of Version {0} Epoch {1} on {2}'.format(psnr_dir.split('/')[-1], ckpt_name, args.dataset_type))
print('Total NAS: ',anomaly_score_total)
