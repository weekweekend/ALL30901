...
def dismap(x,y,landa, name='dismap'):
    ...
    df = pd.DataFrame(res)
    sns.heatmap(df)
    plt.savefig('results/dismap2/{:0>3}.png'.format(name))
    # aaa
    plt.close()
    return True
...
parser.add_argument('--model_dir', type=str,  default='pths/log_3_bs/model_400.pth', help='directory of model')
...
with torch.no_grad():
    for k,(imgs) in enumerate(test_batch):
        ...
        # 帧预测 loss （图像层面的loss）
        mse_imgs = loss_func_mse((outputs[:]+1)/2, (imgs[:,-3:]+1)/2)       #([1, 3, 576, 768])
         
        # 特征重构loss （特征层面的loss）
        mse_feas = fea_loss.mean(-1)                     # ([1, 442368]) 
        mse_feas = mse_feas.reshape((-1,1,576,768))      # ([1, 1, 576, 768])

        # file_path2 = 'results/testb-{:d}.jpg'.format(k)
        # save_image(mseFeas, file_path2)

        # 帧预测loss
        aa = mse_imgs                                           # ([1, 3, 576, 768])
        mse_imgs = mse_imgs.view((mse_imgs.shape[0],-1))        # ([1, 1327104])
        mse_imgs = mse_imgs.mean(-1)                            # ([1])


        # 特征重构loss
        bb = mse_feas                                           # ([1, 1, 576, 768])
        mse_feas = mse_feas.view((mse_feas.shape[0],-1))        # ([1, 442368])
        mse_feas = mse_feas.mean(-1)                            # ([1])
