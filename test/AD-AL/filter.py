    ...
    for  x in range(len(grid)):
        for y in range(len(grid[0])):
            if grid[x][y]==255: 
                ...
                n = dfs(grid,x,y)
                res+=n  #累加岛屿的数量
                points.append(allp[math.floor(len(allp)/2)])
    return res,points
        ...
        if len(p)>0:
          ...
          for [x,y] in p:
            ...
            # 方差归一化
              ...
              var3 = var3[1:]
            # 附近有异常点
            if np.max(var) - np.min(var)>1000 and varsum>0.7:
            # if varsum>0.4 and np.max(var)-np.min(var)>500  and var[0]>0 and var[0]<1000:
              ...
              imm2 = imm1[imm1>0]
              # # 三次确定
              # if abs(np.mean(imm2) - mean_all)<10:
              #   break
              mask2 = imm1
              mask2[mask2>0]=1
              _,bina = cv2.threshold(im1, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_TRIANGLE)
              ...
              if np.mean(imm2) < mean_all:
                ...
                type_yc=1
              bina = bina*mask2
              if abs(np.mean(imm2) - mean_all)<10: break
